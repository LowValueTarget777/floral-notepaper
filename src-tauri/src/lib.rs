...
fn config_save(app: AppHandle, config: AppConfig) -> Result<AppConfig, AppError> {
    ...
    if saved.sync_enabled
        && !saved.sync_server_url.trim().is_empty()
        && !saved.sync_token.trim().is_empty()
    {
        services::sync::request_auto_sync(&app);
    }
    Ok(saved)
...
#[tauri::command]
async fn sync_test_connection() -> Result<SyncStatus, AppError> {
    let store = default_store()?;
    let config = store.load_config()?;
    services::sync::test_connection(&store, &config).await
}
