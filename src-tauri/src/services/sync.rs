...
fn replace_file_atomically(temp_path: &Path, path: &Path) -> Result<(), AppError> {
    ...
    {
        if path.exists() {
            return replace_file_atomically_windows(temp_path, path);
        }
    }
...
#[cfg(windows)]
fn replace_file_atomically_windows(temp_path: &Path, path: &Path) -> Result<(), AppError> {
...
fn status_from_state(config: &AppConfig, state: &SyncState) -> SyncStatus {
    SyncStatus {
        ...
        last_revision: state.last_revision.to_string(),
...
fn sync_http_client() -> Result<&'static reqwest::Client, AppError> {
    ...
    Ok(SYNC_HTTP_CLIENT
        .get()
        .expect("sync HTTP client should be initialized"))
}
...
mod tests {
    ...
    #[test]
    fn reuses_sync_http_client_instance() {
        let first = sync_http_client().expect("build sync client");
        let second = sync_http_client().expect("reuse sync client");
